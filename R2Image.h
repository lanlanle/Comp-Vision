...
class R2Image {
 public:
  ...
  R2Image(const R2Image& image); //copy constructor
  ~R2Image(void);
  ...
  // show how SVD works
  void svdTest();
  vector<double> svdHelper(vector<vector <double> > A, vector<vector <double> > B, int run);

  //frameProcessing
  void FirstFrameProcessing();
  void FrameProcessing(R2Image *otherImage);
  void TrackFeatures(R2Image *otherImage);
  ...
  vector< vector <double> > Harris(double sigma);
  void Sharpen(void);
  void Median(void);
 ...
 private:
  ...
  // bool sortFunc(vector<double>& p1,vector<double>& p2 ) ;
 ...
 private:
  ...
  int width;
  int height;
  R2Image *localFirstImage;
  R2Image *localLatestImage;
};
